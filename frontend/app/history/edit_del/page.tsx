...
export default function EditPatientPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      Swal.fire({
        icon: 'success',
        title: 'สำเร็จ',
        text: `บันทึกข้อมูลผู้ป่วย ${formData.firstName} สำเร็จ!`,
        customClass: {
          popup: isDark ? 'dark:bg-slate-900 dark:text-white border border-slate-800' : '',
          title: isDark ? 'text-white' : '',
          htmlContainer: isDark ? 'text-slate-300' : ''
  ...
  const handleDeletePatient = async () => {
    ...
    const result = await Swal.fire({
      ...
      text: "คุณต้องการลบข้อมูลผู้ป่วยรายนี้ใช่หรือไม่? การกระทำนี้ไม่สามารถย้อนกลับได้",
      icon: 'warning',
              ...
              <div className="bg-slate-50/60 dark:bg-slate-950/50 rounded-[2.5rem] p-10 border border-slate-100 dark:border-slate-800 flex flex-col">
                ...
                <div className="space-y-6 flex-1">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">Patient ID</label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        readOnly 
                        value={patientId}
                        className="w-full bg-blue-50/50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/50 p-4 pl-12 rounded-2xl font-mono font-bold text-blue-600 dark:text-blue-400 outline-none" 
                      />
                    </div>
                  </div>
